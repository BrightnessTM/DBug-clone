...
module.exports = {
    Send: async (client, message, title, steps, actual, expected, clientSettings, systemSettings) => {
        ...
        client.channels.cache.get(config.channels.approvalQueue)
        .send(`From: <#${message.channel.id}>`, {embed}).then(msg => {
            DB.SaveReport(message, msg, message.channel.id, title, steps, actual, expected, clientSettings, systemSettings)
        })
    ...
    UpdateStance: async (client, report) => {
        const message = await client.channels.cache.get(config.channels.approvalQueue).messages.fetch(report.messageId)
        ...
        if(report.approves.length >= 3 || report.stance === "Approved") ApprovedBug(client, report, embed)
        if(report.denies.length >= 3 || report.stance === "Denied") DeniedBug(client, report, embed, message.content)
...
function ApprovedBug(client, report, oEmbed){
    ...
    client.channels.cache.get(report.platform).send(embed).then(msg => {
        Report.findOneAndUpdate({ _id: report._id }, { stance: "Approved", messageId: msg.id}, function (err) {
            if (err) {
              console.error(err);
            }
          });
    ...
    client.channels.cache.get(config.channels.approvalQueue).messages.fetch(report.messageId).then(msg => msg.delete())
}
